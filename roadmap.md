...
### 🎯 LO QUE QUEDA PENDIENTE DE IMPLEMENTACION (medido el 11/09 sobre `b79d904`; `648b62f` solo toca `README.md`; **lo de la hora re-medido el 11/09 por la tarde sobre `68dd2c5`**, el merge de `D-26`)
...
| 🆕 **1.13** | ~~🟠 **EN CONSTRUCCION (worktree, 11/09 por la tarde; NO esta en `main`)**~~ *(11/09 por la noche: **CONSTRUIDA y en `main`** — commit `merge(D-21 (1))`; `reloj_horaFiable()` en las dos puntas, plazo `HORA_CADUCA_MS` = 320 s derivado en `reloj.h` y recalculado por `reloj_04`; **sin banco**)* 🟠 **`D-21` pieza (1) — AMBAR INTERMITENTE EN LA PUNTA CUYA HORA CADUCO, dentro del Degradado.** Es la cura de **`H1`** del arquitecto (§3.16 `N-162`): con el `J17` del Maestro mudo su hora corre sobre el HSI, la radio la empuja al Esclavo, y **al caer la radio el Esclavo adopta su `DS3231` y el Maestro sigue con la derivada**: verde contra verde en cada ciclo. `D-26` (4) protege el INSTANTE del salto, no el desfase que deja. **Decidida el 07/09** (`D-21` (1)) **y sin construir hasta hoy**; `D-26` la vuelve urgente porque tumba la premisa de los «MESES» (fila 2.7). **Bloquea campo, no el merge de `D-26`** | `D-21` (1), `D-26`, `N-162` `H1` | lo dira su diff al integrarse | en `main`, `ESP32_Expansion/src/main.cpp` lo confiesa en su cabecera: *«NO declara vieja la hora que tiene ni cae a ambar por ello: eso es la pieza (A) de D-21, sin construir»*; `horaEsp32Vigilar()` publica `$ALARM …EVENTO:HORA_ESP32,CAUSA:J17_MUDO…` y **no hace nada mas** |
...
| 🆕 **1.16** | 🟡 **Los residuos que dejaron escritos los agentes al integrar `D-26`/`D-21` (1) el 11/09 por la noche** — ninguno es de seguridad; todos son «el siguiente que lo lea se equivoca»: **(a)** los rotulos `RENDIDO 48h` (`degradado_textoEstado()`, `menu.cpp`) **mienten** cuando la rendicion es por la hora caducada; **(b)** los comentarios de `Esclavo/src/botones.cpp`/`.h` siguen diciendo «una camara por poste» (`D-25`: dos) — los del Maestro ya se corrigieron, sin ancla; **(c)** el bloque D de `dos_puntas/orquestador.cpp` mide la reanudacion tras corte con un modelo de RTC que el firmware ya no escribe (`N-162` `H8`); **(d)** aviso del compilador de truncado en `reloj_textoHora()` (`buf[6]`), anterior a esto; **(e)** `documentos_06`: anadir *«la app no pone la hora en el poste 2, nunca»* a sus frases derogadas (la corrigieron ~20 documentos el 11/09 y nada impide que vuelva) y retirar el comentario que atribuye su motivo 7 a una rama `!propagada` que ya no existe; **(f)** en la tabla de rutas de `ARQUITECTURA.map`, `reloj_02` no lista `M:bluetooth.h` y `app_02` no lista `M+E:modos.h`; y la tabla de la compuerta de ese fichero va fechada al 07/09 (cifras viejas a proposito, pero nadie la refresca); **(g)** `D-21` pieza (2): el `$ALARM ...CADUCADA` sale UNA vez y el STM32 no sabe cuando se conecta el telefono — quien llegue despues no la ve hasta `D-23` (fila 1.3) | `N-162`, `D-21`, `D-25`, `CLAUDE.md` §14 | `{Maestro,Esclavo}/src/{modo_degradado,menu,botones}.cpp`, `Validacion_Automatico/dos_puntas/orquestador.cpp`, `banco/packs/documentos_06_*.py`, `ARQUITECTURA.map` | los informes de los agentes del 11/09 por la noche, re-medidos al integrar (`fdccd5b`) |
