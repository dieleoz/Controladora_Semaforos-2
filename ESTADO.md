...
**Compuerta:** ✅ **15 PASS | 0 FALLA | 0 ABORTADO** (Exit code: 0) · medida sobre `3733544`, **árbol LIMPIO** · Maestro: 85.8% Flash (56.260 de 65.536 B → **9.276 B libres**) · Esclavo: 63.9% Flash · Repetidor: 20.6% Flash · 348/348 en 34 packs · 271/271 pantalla · 71/71 automatico · 29/29 ciclo · app: 32 unitarios + 59 jsdom + 57 funcional · `evidencia/2026-08-28_compuerta.txt`
...
| ~~**FLASH**~~ | ✅ Bajada | Maestro del **93.5 % al 85.6 %** (56 084 / 65 536 B): **9 452 bytes libres**, mas del doble que antes. No se toco una linea de firmware: `U8x8lib.cpp` referenciaba `TwoWire::setClock()` y el enlazador arrastraba `Wire` → el HAL de I2C entero —**5 160 B de flash y 352 de RAM en cada punta, por un bus que el equipo no tiene**—. Causa leida en `firmware.map` y delta medido quitando y poniendo las banderas sobre el mismo arbol. Lo vigila `flash_01_lastre`, que ademas exige quitarlas el dia que entre el `PCF8574`. **Queda margen sin gastar:** `ncenB14` y `ncenB12` se usan una vez cada una y suman 3 894 B (N-70). ⚠️ **Las cifras de esta fila son las del día de N-70 y se conservan como historia:** la vigente es la del acta del 28/08 — **85.8% (56.260 de 65.536 B), 9.276 B libres** |
