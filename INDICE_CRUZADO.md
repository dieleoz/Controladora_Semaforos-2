...
| el mismo sobre **`.md`** | *«cero»* | **tres** — `OPTIMIZACIONES.md:445` y `:1634`, y `roadmap_hist.md` —esa cita era por número de línea y ya estaba caducada antes de la partición del 07/09— |
...
| **`BLQ-2`: el cristal `Y2` de la SEGUNDA tarjeta sin diagnosticar** | `ESTADO.md` · `roadmap_hist.md` · `11_Manual_Instalacion_RTC...` | responsable (banco, `B5`) | — |
| **`N-106`: el ámbar de la app no sacaba al Esclavo del Degradado** | `ESTADO.md` · `roadmap_hist.md` · `OPTIMIZACIONES.md` · 7 documentos | responsable decide **qué debe hacer**; técnico escribe el arnés | ✅ **CERRADO — y el pack está VIGILANDO, no celebrando.** `esclavo_08_ambar_en_degradado` da hoy **8/8** *(corrido solo el 02/09)*, y **nació EN ROJO a propósito** (`829c457`): por eso su verde vale. **Cuatro de sus ocho comprobaciones son controles negativos**, y uno existe justo para que el arreglo no entrara por **una sola** de las dos puertas. Medido en `Esclavo/src/bluetooth.cpp`: las dos puertas del ámbar consultan `degradado_gobiernaLuz()` **dentro de su bloque** (`:316`, `:403`), tienen rama para `degradado_rendicionEnCurso()` (`:340`, `:417`), y hay un **envoltorio** (`:260-273`) que devuelve lo que `degradado_salir()` —que es `void` y abandona en silencio— no sabía decir. 🔴 **Lo cerrado es la incoherencia entre las dos vías, NO la decisión vial**: *qué debe hacer* el ámbar de la app en Degradado sigue siendo del responsable |
| **`N75-1` … `N75-4`** | `ESTADO.md` · `17_...` §3.4 · `roadmap_hist.md` | responsable / técnica | — |
...
| ~~**`05_Funcional/ENCARGO_SESION_BANCO.md`**~~ → **`05_Funcional/historico/ENCARGO_SESION_BANCO_ARCHIVADO_07-09.md`** | 1, y es un **histórico cerrado** | ~~el encargo de la sesión de banco, que es **el bloqueante del proyecto**~~ → 🗄️ **archivado el 07/09 como acta de la 1.ª noche.** El protocolo vigente es `05_Funcional/Guia_Cableado_y_Pruebas_Banco.html` (44 pasos) | ✅ resuelto por archivado: ya no compite con la Guía |
...
| **P-6** | ~~🔴 **`05_Funcional/ENCARGO_SESION_BANCO.md`**: el encargo de la sesión de banco… colgando solo de un histórico cerrado~~ → 🟢 **CERRADO EL 07/09 POR ARCHIVADO.** Ese encargo **ya no es el vigente**: se movió a `05_Funcional/historico/ENCARGO_SESION_BANCO_ARCHIVADO_07-09.md` como acta de la 1.ª noche | — | El protocolo de banco vigente es **`05_Funcional/Guia_Cableado_y_Pruebas_Banco.html`** (44 pasos, 05/09), enlazado desde `05_Funcional/README.md` | ✅ hecho — y el problema real no era el puntero: eran **dos encargos vigentes a la vez** |
