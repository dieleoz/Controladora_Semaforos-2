...
const unsigned long TIMEOUT_ACK_MS = 2500; // SFTY-6: 1.5s timeout per retry
...
void coordinador_actualizar() {
  ...
  if (millis() - tUltimoPing > 1000) { // SFTY-8: PING cada 1.0s
    protocolo_enviarPaquete(0x04); // PING
    tUltimoPing = millis();
    static unsigned long tFalloCom = 0;
    if (!handshakeOk) {
      if (tFalloCom == 0) tFalloCom = millis();
      else if (millis() - tFalloCom > 3000) { // SFTY-8: Fallo tras 3 segundos sin heartbeat
        estadoC = C_FALLO;
      }
    } else {
