...
|---|---|---|
| Compilación Maestro (STM32F103) | **41880 B de Flash — 63.9 %** de 65536 B (quedan **23656 B**) | `pio run` |
| Compilación Esclavo (STM32F103) | **36200 B de Flash — 55.2 %** de 65536 B | `pio run` |
...
| Banco por packs | 🔴 **1415/1416 comprobaciones**, **79 packs** — 78 PASS, **1 FALLA**. **Este documento se FIRMA, así que la frase que había aquí —*«D-14, D-20, D-21, D-22 y D-23 integradas y ancladas»*— se retira por FALSA.** Hoy el único rojo es `decisiones_01_anclas` por **`D-22`**, la única decisión vigente sin construir, y **necesita una tarjeta delante**: no se cierra con teclado. `D-23` y `D-33` sí están construidas; `D-14` no se instala y `D-30` quedó recortada con el LCD | `banco/correr.py` |
...
| Arnés del Degradado a dos puntas | **69/69** | cada punta con su reloj — desde el 11/09, con el `reloj.cpp` REAL de las dos puntas |
...
> |---|---|---|
> | ~~Maestro: 42.620 B (65,0 %)~~ · ~~41800 B (63.8 %)~~ | ~~41228 B (62.9 %)~~ · ~~41328 B (63.1 %)~~ · **41880 B (63.9 %)** | 🔴 **El error grave, y su final.** Quien planificase con el 65 % creería tener **~23 KB libres** cuando quedaban **7136 B**. Los ~23 KB existen desde el 13/09, pero por otro motivo: **se retiró el LCD** (`D-32` (1)). Hoy quedan **24308 B** |
> | ~~Esclavo: 15.480 B (23,6 %)~~ · ~~36648 B (55.9 %)~~ | ~~35976 B (54.9 %)~~ · ~~36092 B (55.1 %)~~ · **36200 B (55.2 %)** | Casi el triple de ocupación real |
