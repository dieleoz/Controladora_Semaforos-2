# 17 (historico) — Arquitectura y decisiones abiertas: la cronica, lo cerrado y lo derogado

> 🛑 **ESTE FICHERO NO ES SPEC, Y NO GANA NUNCA.** La spec de cobre viva es
> [`17_Arquitectura_28-08_y_Decisiones_Abiertas.md`](17_Arquitectura_28-08_y_Decisiones_Abiertas.md),
> en esta misma carpeta. Si un parrafo de aqui contradice a aquel, **el caducado es el de aqui**:
> un documento congela el dia en que se escribio, y un documento legacy **no reabre una decision**
> (`CLAUDE.md` §11).

**Que hay aqui y por que.** El 12/09/2026 aquel fichero tenia **4.182 lineas** y ya no se leia
entero: se citaban trozos, y cada lector derivaba su propia version. Se partio. Aqui esta lo que
salio, que es de tres clases:

- **la CRONICA** — como se descubrio cada cosa, que se refuto y que paso tal dia;
- **las DECISIONES YA CERRADAS** cuyo resultado ya vive en la tabla del fichero vivo — se conservan
  **con sus alternativas y sus costes**, porque una decision entre alternativas solo se puede
  revisar si las alternativas siguen escritas;
- **las MEDIDAS DEROGADAS** — que **se mudan, no se borran**: una causa que desaparece en silencio
  se vuelve a proponer, y la segunda vez nadie recuerda que se comprobo (`CLAUDE.md` §7.4).

🔴 **NADA SE HA REESCRITO NI RESUMIDO.** Todos los bloques de abajo son **literales**, en el mismo
orden en que estaban. Lo unico que no venia de alli son los rotulos **`▸`** que dicen de que
apartado sale cada bloque, y la cuenta del final. **Las medidas de cobre —con su fecha, su
instrumento y el firmware que habia dentro— viajaron con su bloque, sin tocar un numero.**

---

▸ **de la CABECERA — la lista de revisiones y las dos actas de compuerta que citaba** *(lineas 2-25 del fichero de partida)*
...
▸ **de la CABECERA — el censo de citas que obligo a citar por SIMBOLO** *(lineas 28-49 del fichero de partida)*
...
▸ **REVISION DEL 07/09 (Modo Degradado) y REVISION DEL 05/09 (`D-1` y `D-16`), enteras** *(lineas 101-444 del fichero de partida)*
...
▸ **de la REVISION DEL 03-04/09 — la cuenta del informe de banco y la tabla de veredictos** *(lineas 446-491 del fichero de partida)*
...
▸ **de la REVISION DEL 03-04/09 — las dos conclusiones de la medida de `J16`** *(lineas 508-551 del fichero de partida)*
...
▸ **de la REVISION DEL 03-04/09 — el desarrollo de `D-12` (el contacto seco)** *(lineas 557-630 del fichero de partida)*
...
▸ **REVISIONES del 03-04/09 (Bluetooth), 04/09 tarde, 04/09 noche, 04-05/09 noche del banco y 31/08** *(lineas 658-1391 del fichero de partida)*
...
▸ **de §1.6 — el censo del 31/08 del mando** *(lineas 1632-1651 del fichero de partida)*
...
▸ **de §1.6 — *«el mando vive entero en `A` y `B`»* y su caveat** *(lineas 1675-1690 del fichero de partida)*
...
▸ **de §1.7 — la ambiguedad «Camara 1/2», el `grep` sin `-E`, y las dos filas tachadas** *(lineas 1729-1813 del fichero de partida)*
...
▸ **de §1.7.bis — como convergen las tres entradas, y las citas por numero caducadas** *(lineas 1899-1944 del fichero de partida)*
...
▸ **de §1.7.bis — el vigilante de `D-13` fase 1 y el caducado de `presenciaActual`** *(lineas 1953-1992 del fichero de partida)*
...
▸ **§2.2, §2.3, §2.4, §2.5, §2.6, §2.7 y §2.8, enteras** *(lineas 2066-2761 del fichero de partida)*
...
▸ **del encabezado de §3, y §3.1 entera** *(lineas 2767-2854 del fichero de partida)*
...
▸ **§3.3, §3.4 y §3.5, enteras** *(lineas 2873-3232 del fichero de partida)*
...
▸ **§3.7 entera** *(lineas 3280-3337 del fichero de partida)*
...
▸ **§A — las cinco recetas de multimetro, M1 a M5** *(lineas 3596-3723 del fichero de partida)*
...
▸ **§B — las cinco fichas del orden y el segundo bloque de documentos falsos** *(lineas 3732-3967 del fichero de partida)*
...
▸ **Anexo, y el cierre del documento** *(lineas 4008-4182 del fichero de partida)*
...
## La cuenta, que es lo unico que demuestra que no se borro nada

**Se publica porque una afirmacion de que «esta todo» no vale nada sin la resta.** El fichero de
partida tenia **4.182** lineas el 12/09/2026, y **cada una de ellas esta hoy en uno de los dos
ficheros: ninguna en los dos, y ninguna en ninguno.**

| | lineas del fichero de partida |
|---|---|
| `17_Arquitectura_28-08_y_Decisiones_Abiertas.md` **antes** del corte | **4.182** |
| se quedaron en el fichero VIVO | **946** |
| se mudaron a ESTE fichero | **3.236** |
| **suma** | **4.182** ✅ **cuadra exacta** |

**Y lo que cada fichero tiene ADEMAS**, declarado aparte para que la resta de arriba siga siendo
limpia — es andamiaje nuevo, no texto de partida:

| fichero | lineas de partida | andamiaje nuevo | **total hoy** |
|---|---|---|---|
| `17_Arquitectura_28-08_y_Decisiones_Abiertas.md` (vivo) | 946 | 196 *(cabecera con el tope, los tres avisos, y las lineas «esto se fue a…»)* | **1.142** |
| `17_hist_Arquitectura.md` (este) | 3.236 | 123 *(esta cabecera, los rotulos `▸` y esta cuenta)* | **3.359** |

> **La cuenta se puede repetir, y en dos comandos:**
> `wc -l 05_Funcional/17_Arquitectura_28-08_y_Decisiones_Abiertas.md 05_Funcional/17_hist_Arquitectura.md`
> — y el fichero de partida es el que hay **en el commit anterior a la particion**:
> `git show <ese commit>:05_Funcional/17_Arquitectura_28-08_y_Decisiones_Abiertas.md | wc -l` -> **4.182**.
