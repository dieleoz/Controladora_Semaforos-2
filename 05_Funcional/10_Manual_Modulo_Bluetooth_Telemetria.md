...
**CONSULTAR el reloj sin cambiarlo (`CMD:LEER_RTC`)**, forzar
...
> 🔵 **11/09 (`D-26`, construida en `68dd2c5`, sin banco) — ESTA CAJA ESTÁ CADUCADA EN SU NÚCLEO, Y
> SE CORRIGE HACIA LA DECISIÓN:** «ajustar el reloj» **vuelve** a la lista del poste 2. Con radio, su
> controladora hace caso a la hora del Maestro; **sin radio, toma la de su propio ESP32, que el técnico
> le pone desde el teléfono en su gabinete** —es lo que se hace cuando salta la alarma de radio—. La
> prohibición de abajo ya estaba tachada en la propia fila `D-20` el 07/09 por la noche (*«la barrera
> es la SOBREESCRITURA»*). Qué ve y qué hace el técnico: `14_Manual_App_Movil_IOT_VIAL.md` §5.3.bis.
>
> ~~🛑 **07/09 (`D-20`) — «AJUSTAR EL RELOJ» SALE DE ESTA LISTA, Y ES LA CORRECCIÓN MÁS DURA DEL
> APARTADO: ES JUSTO LA FRASE DONDE HACÍA FALTA LA PROHIBICIÓN.**~~
>
...
> **Hay UNA sola fuente.** Un `SET_RTC` dirigido al Esclavo **se rechaza**: no es una sincronización,
> **es una segunda fuente de hora**.~~ y el STM32 de cada punta la recibe **de su propio ESP32**; el
> del poste 2, **con radio**, hace caso a la del Maestro. Filas **`D-20`** y **`D-26`** de `DECISIONES.md`.
>
> ```
...
>
> ✅ **Lo que NO cambia: `CMD:LEER_RTC` se sigue mandando A LOS DOS POSTES.** ~~`D-20` prohíbe
> **escribir** la hora en el poste 2;~~ **leerla no escribe nada** ~~, y con `D-20` construida es la
> única forma de comprobar que la siembra del Maestro llegó de verdad~~ *(11/09: la radio no escribe el
> `DS3231` del poste 2; lo que mide es el salto que dará el poste 2 al perder la radio)*.
>
> ~~🔴 **DECIDIDA Y SIN CONSTRUIR.**~~ *(construida el 11/09 en `68dd2c5`)* **Medido el 07/09, corrido antes de publicarlo** *(crónica)*:
>
> ```bash
...
> `SET_RTC` **no nombra al Esclavo ni una vez**: hoy lo atiende venga por donde venga *(11/09: y con
> `D-26` es lo correcto)*. ~~Falta también el mando **ESP32 → STM32** que siembre la hora en la
> controladora (el camino físico existe, `enlace_stm32.cpp`; el mando no).~~ *(existe desde `68dd2c5`:
> `CMD:HORA_ESP32`, `siembra.cpp`)*
>
> ~~⚠️ **Mientras tanto NO hay ninguna otra vía de poner en hora el poste 2, así que hoy se sigue
> visitando y sincronizando allí. Eso es un ESTADO DECLARADO, no la arquitectura**, y muere el día
> que `D-20` se construya.~~ *(No muere: es el procedimiento de `D-26` (5).)*
>
> 🔵 **Y la regla de operación que sale de todo esto: EL POSTE 2 SE PONE EN HORA EN LA PUESTA EN
> MARCHA** ~~**, NO DURANTE LA AVERÍA.** Con `D-20` dentro, el único camino al reloj del poste 2 pasa
> por la **radio**, y la radio se cae justo cuando hace falta el Modo Degradado — que es el modo que
> **exige hora**.~~ **Y CON LA RADIO CAÍDA, ALLÍ MISMO** (`D-26`). ✅ **Su `DS3231` tiene pila y
> conserva la hora que ya tenía. Perder la radio no es perder la hora.** Por eso se pone también
> **antes**: en la puesta en marcha, al cambiar la pila de ese módulo, y tras cualquier
...
### Pantalla 5: AJUSTES & ~~ASISTENTE COURIER RTC~~ **RELOJ**

> 🛑 **CORREGIDO EL 07/09 — `D-20` cambia a QUIÉN se le pone la hora, y el Courier RTC queda
> DEROGADO.** ~~Ver la caja larga del `§1` (*«lo que la app NO puede hacer»*): **la app pone la hora en
> el POSTE 1 y sólo en el POSTE 1**; el poste 2 la recibe del Maestro por radio. **Hay UNA sola
> fuente.** 🔴 **SIN CONSTRUIR** — hoy el poste 2 se sigue poniendo en hora visitándolo, y eso es un
> estado declarado, no la arquitectura.~~ 🔵 **11/09 (`D-26`, `68dd2c5`, sin banco): la hora se pone en
> LOS DOS postes**; el poste 2 hace caso a la del Maestro con radio y a la de su ESP32 sin ella. El
> Courier sigue derogado. `14_Manual_App_Movil_IOT_VIAL.md` §5.3.bis.

* **Sincronización Directa de Hora:** Ajuste del reloj `DS3231` **del ESP32 del poste conectado** con la hora del teléfono móvil. ⛔ ~~del nodo conectado~~ — se leía como el reloj del STM32, que va sobre `Y2` y está confirmado muerto (`N-17`). ~~🛑 **07/09 (`D-20`): esta orden es DEL POSTE 1.** Dirigida al Esclavo **se rechaza** — sería una segunda fuente de hora.~~ 🔵 **11/09 (`D-26`): de los dos postes.** La atiende el puente, que la pasa a su controladora (`CMD:HORA_ESP32`); ya no cruza al STM32.
* 🔍 **Consulta de Hora (`CMD:LEER_RTC`, `D-17`):** ✅ **se sigue usando en LOS DOS POSTES.** Lee sin escribir ~~, así que `D-20` no la toca; con `D-20` construida es **la única forma de comprobar que la siembra del Maestro llegó al reloj del poste 2**~~ *(11/09: mide el salto que dará el poste 2 al perder la radio — la radio no escribe su `DS3231`)*. Ver `§4.1`.
* ~~**Modo Asistente Courier RTC (Sincronización Puente sin Radio):**~~ 🛑 **DEROGADO EL 07/09 (`D-20`). Se conserva tachado con su motivo para que no se vuelva a proponer:**
  ...
  **El motivo, y no es que estuviera mal hecho: la hora que ese asistente inyectaba en el poste 2
  NO era la del Maestro, era la DEL TELÉFONO** — al Maestro no se le manda nada durante todo el
  procedimiento. Eso no es una sincronización: **son dos fuentes de hora**, y es exactamente lo que
  `D-20` prohíbe. La compensación del viaje era correcta; **lo que estaba mal era de dónde salía la
  hora**. 🔴 **El botón sigue en la APK — `D-20` decide la autoridad, no ha retirado código. No se
  use.**
...
> dos respuestas—, y dejan un `$EVENT` con `DETALLE:SET_RTC_LO_ACUSA_EL_PUENTE`.~~ 🔵 **11/09
> (`D-26`, `68dd2c5`): la orden ya NO LLEGA al STM32** —el puente se la queda— y ese `$EVENT` ya no
> existe. Lo que la controladora recibe es `CMD:HORA_ESP32:…` con la hora releída del `DS3231`, sin PIN
> y sin respuesta; en el diario queda `HORA_ESP32_SEMBRADA` (o, en el poste 2 con radio,
> `HORA_ESP32_IGNORADA_MANDA_RADIO`) sólo en el cambio. `14_Manual_App_Movil_IOT_VIAL.md` §5.3.bis.
>
> ~~🛑 **07/09 (`D-20`) — ESTO SIGUE SIENDO CIERTO DEL STM32, Y YA NO BASTA.** *«Las dos puntas
> consumen la orden en silencio»* describe al STM32 y no cambia. Lo que `D-20` añade es **una punta
> más arriba**: **el PUENTE del poste 2 tiene que RECHAZAR el `SET_RTC` con motivo**, no atenderlo.
> Un `SET_RTC` dirigido al Esclavo **no es una sincronización, es una segunda fuente de hora**, y
> consumirlo callado no es lo mismo que negarlo — **el silencio no le dice al técnico que su orden
> no valía.**~~ *(Derogado: ya tachado en la propia fila `D-20` el 07/09 por la noche, y `D-26` manda
> ponerle la hora al poste 2 cuando cae la radio. El puente no rechaza nada: decide la controladora.)*
>
> 🔴 **SIN CONSTRUIR, y se mide en vez de suponerse (07/09, corrido antes de publicarlo):**
>
> ```bash
> $ grep -c "ESCLAVO\|Esclavo\|esclavo" 01_Firmware/ESP32_Expansion/src/despachador.cpp
> 0
> ```
>
> El puente es **el mismo firmware en los dos postes** y **no nombra al Esclavo ni una vez** en el
> fichero que decide qué hacer con un `SET_RTC`: hoy lo atiende venga por donde venga. Ver la caja
> de `§1`.
>
...
>
> ⚠️ **Un cruce tiene DOS relojes y ~~nada los sincroniza~~ hoy nada los sincroniza.** Por eso la app
> **compara**: guarda el error de cada poste contra el reloj del celular y los resta, así que **la
> caminata entre postes se cancela**. Con un solo poste no publica desfase: **dice cuál falta**. La
> lista vive en memoria y **no sobrevive a que Android cierre la app**.
>
> 🛑 **07/09 (`D-20`) — dónde poner el remedio de estos ocho motivos, porque NO es el mismo en los
> dos postes.** Cuatro de los ocho terminan en *«sincronizar»*, y sincronizar es **escribir la
> hora**:
>
> * **POSTE 1 (Maestro):** se sincroniza ahí mismo. Es **la fuente**, y es lo correcto.
> * **POSTE 2 (Esclavo):** ~~🛑 **no.** `D-20` decide que la app **no pone la hora en el poste 2,
>   nunca** — sería una segunda fuente. Con `D-20` construida, esos motivos se resuelven **poniendo
>   en hora el Poste 1 y dejando que lo siembre por radio**, y esta consulta pasa a servir para
>   **comprobar que llegó**. ⚠️ **SIN CONSTRUIR:** hoy se sigue sincronizando allí, visitándolo.~~
>   🔵 **11/09 (`D-26`): también ahí mismo.** Estos motivos son de **su** `DS3231`, y la radio no lo
>   escribe: sólo se arreglan sincronizando en ese poste.
> * 🔧 **Lo que no cambia en ningún poste:** *«oscilador parado»* es **la pila del `DS3231` de ese
>   poste** —**no** la `CR2032` de la controladora—, se cambia en el poste donde salga, y **no hay
>   radio que arregle una pila**.
>
> 🔵 **Y la frase que sí se conserva entera, porque `D-20` la usa como premisa: los dos ESP32 NO se
> hablan entre sí.** Por eso la comparación la tiene que hacer la app visitando los dos postes, y por
> eso la hora sólo puede viajar **por la radio entre los STM32**.
...
| `SET_RTC:…` | ✅ **con PIN** *(11/09: el PIN ya no lo comprueba nadie, `D-26` (1))* | ~~🛑 **RECHAZADO con motivo** (`D-20`, 07/09)~~ ✅ **igual que en el Maestro** (`D-26`, 11/09) | ~~🔴 **Esta fila decía `✅ con PIN` en las DOS puntas, y era la peor del manual.** Poner la hora **no es simétrico**: la del poste 2 **sale del Poste 1 por radio**, así que un `SET_RTC` dirigido al Esclavo **no es una sincronización, es una SEGUNDA FUENTE de hora**. ⚠️ … **el que tiene que rechazarla es el PUENTE del poste 2** — y eso **está SIN CONSTRUIR**~~ 🔵 **11/09 (`D-26`, `68dd2c5`): la fila vieja volvía a tener razón.** La atiende el puente de cada poste y **no cruza** al STM32. Lo asimétrico es qué hace cada controladora con la hora que su ESP32 le pasa: el Maestro la toma siempre; **el Esclavo, sólo sin radio** (`§1`, `14_…` §5.3.bis) |
| `TEST_LEDS` | ✅ con PIN | 🛑 **RECHAZADO** | Ver abajo |
| 🔍 `LEER_RTC` | — **no llega al STM32** | — **no llega al STM32** | ✅ **`D-20` NO lo toca, y va escrito aquí para que nadie lo retire junto con el `SET_RTC`:** consultar el reloj **no escribe nada**, así que **se sigue mandando A LOS DOS POSTES**. ⚠️ **Pero no es una fila como las otras: esta orden no la atiende ninguna de las dos puntas del STM32** — **el puente se la queda porque el reloj es suyo**, y contesta con `NODE:PUENTE`, **sin PIN**. Reenviada al STM32, las dos contestarían `$ERR,CMD:AUTH_FAILED,DESC:PIN_INVALIDO`. `D-17`, y su tabla de motivos está en `§4.1` |
...
> 🔴 **NO ERA UNA SINCRONIZACIÓN: ERA UNA SEGUNDA FUENTE DE HORA.** La hora que este asistente
> inyectaba en el poste 2 **no era la del Maestro, era la DEL TELÉFONO** — durante todo el
> procedimiento **al Maestro no se le manda nada**. La compensación del viaje era correcta; **lo que
> estaba mal era de dónde salía la hora**.
>
> **Lo vigente: LA AUTORIDAD DE LA HORA ES EL ESP32** (`D-20`) **de cada poste** (`D-26`, 11/09,
> construida en `68dd2c5`, sin banco). ~~La app se la da al **ESP32 Maestro**; ése al **ESP32
> Esclavo**; el STM32 de cada punta la recibe **de su propio ESP32**. **Hay UNA sola fuente**, y **la
> app NO pone la hora en el poste 2. Nunca.**~~ **Con radio, el poste 2 hace caso a la del Maestro; sin
> radio, a la de su propio ESP32**, que el técnico le pone desde el teléfono en su gabinete.
>
> ```
> ESP32-M  ->  STM32-M  ->  radio  ->  STM32-E            (con radio; los STM32 son CARTEROS)
>                           ESP32-E  ->  STM32-E           (SIN radio 25 s)
> ```
>
> ~~🔴 **DECIDIDA Y SIN CONSTRUIR** — la medida está en `§1`. **Mientras tanto, y sólo mientras tanto,
> al poste 2 se le sigue poniendo la hora visitándolo con «Sincronizar reloj». Eso es un ESTADO
> DECLARADO, no la arquitectura**, y muere el día que `D-20` se construya.~~ *(11/09: visitar el poste 2
> con «Sincronizar reloj» es el procedimiento de `D-26`, no un estado temporal.)*
>
...
> hora**.~~ **Y CON LA RADIO CAÍDA, ALLÍ MISMO** (`D-26` (5)). ✅ **Su `DS3231` tiene pila y conserva la
> hora: perder la radio no es perder la hora.**
>
