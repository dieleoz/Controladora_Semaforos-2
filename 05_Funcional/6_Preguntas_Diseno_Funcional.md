...
1. ~~**Topología de Cámaras (CERRADO):** … a las entradas optoacopladas `PB9` y `PB13`.~~
   ...
   |---|---|---|---|
   | `CAM_DEMANDA_PIN` | `PB0` | `J14` — con `R64` 10 kΩ + `C25` 100 nF | ~~✅ **cableable hoy**~~ → 🟠 **07/09: VIVO Y SIN CÁMARA.** `D-2`/`D-3`: ~~**las dos cámaras del cruce van a `J16` p10 y p12, una por poste**~~ 🔴 **`D-25`, 11/09: son CUATRO cámaras, DOS por poste, en `J16` p10 y p12 de cada equipo**, y **`J14`/`PB0` queda libre**. El motivo no es el conector: **`CAM_CIEGA`/`CAM_PEGADA` miran `J16` y no miran `J14`** — una cámara aquí funciona y **nadie sabría que se estropeó**. ~~Reservado a un posible fin de carrera de barrera~~ 🔴 **`D-27`, 11/09: `J14` queda LIBRE y sin cablear — el fin de carrera NO se instala en este despliegue, y mientras el firmware lea `PB0` como demanda aquí no se conecta nada** |
