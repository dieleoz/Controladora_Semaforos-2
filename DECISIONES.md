...
| **D-33** | 🔴 **LA CÁMARA VETA LA BAJADA DE LA PLUMA, Y LA PLUMA BAJA **3 SEGUNDOS** DESPUÉS DEL ROJO.** *(el número lo fija el responsable el 14/09, no sale de una cuenta: con el despeje al mínimo deja **7 s** de un tramo dimensionado para 10)* Las dos mitades del mismo motivo, dicho por el responsable: *«es su sensor de presencia; si no baja por la cámara da igual, es justo ese el punto de la función de la cámara: que la barrera no se lleve una moto o un carro»*. **Deroga `SFTY-28`** en su parte de *«la talanquera sigue a la luz, nunca al revés»*: sigue a la luz **salvo que haya presencia**. **Veta CUALQUIERA de las dos cámaras** —es su sensor de presencia, no un voto—. Y **el fallo va SIEMPRE hacia la barrera ARRIBA**: ante error o falsa alarma **no baja**, y **la app pide AJUSTE DE CÁMARA**. Si llegan muchas falsas alarmas, la app lo pide igual. 🆕 **Y el operador podrá, desde la app en modo administrador, RETIRAR la barrera de la lógica, poste a poste.** | 14/09 · decidida por el responsable | **Cierra `A-1.bis`**, abierta desde el 05/09, y **`SPEC_1` §12.1**. El comportamiento, el umbral de «muchas» y los modos de fallo, en `SPEC_1` y `SPEC_5`. 🔴 **Y la premisa que esto DEROGA, porque estaba escrita en el fuente y decía lo contrario:** `semaforo.cpp` afirma que *«una pluma arriba con la luz en rojo es PEOR que no tener barrera, porque el conductor confía en ella»*. El responsable, 14/09: **«esas barreras son casi de adorno; el que manda es el semáforo y su estado»**. Con esa premisa **un veto NO para el ciclo**: es local al poste que lo tiene, la otra punta abre su verde con normalidad y el cruce **no se va a ámbar**. Lo que el veto protege es a quien está debajo de ESA barrera, nada más | `A-1.bis` · de `SFTY-28`, el «nunca al revés» · la fase 2 de `D-13` deja de estar bloqueada |
