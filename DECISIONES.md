...
| ~~**A-15**~~ | ✅ **RESUELTA el 08/09: UNA HORA.** El ESP32 siembra la hora a su STM32 **cada hora**, **reusando `INTERVALO_SYNC_MS`** —que ya es una hora para el reenvío Maestro→Esclavo—, así que **hay UN solo número en el sistema y no dos**: dos cadencias distintas para la misma hora es lo que nadie recuerda mantener. Nace de *«no cada ms, algo con sentido en horas, días o meses»*, y **corrige el «cada 2 s» que estaba escrito, que lo puse yo y no se sostenía**: sembrar por un cable cada dos segundos para corregir una deriva que se mide en minutos es atosigar el enlace sin motivo | alta — **ya se puede escribir la siembra** | ⚠️ **Y lo que esta cadencia NO arregla, para que nadie la lea como si cerrara el margen del cruce: en Degradado NO SE SIEMBRA NADA** —no hay radio, que por eso se entró, ni teléfono—, así que las dos puntas corren libres **hasta 48 h** sobre el HSI, con una deriva de **~29 min a ~1,2 h** contra los **29 s** que aguanta el cruce. **Eso lo decide `A-16`, no esto** | ⬇️ 🔴 **CORREGIDA el 14/09:** ~~*«en Degradado NO SE SIEMBRA NADA»*~~ → **si se siembra**, desde el ESP32 del propio poste y cada **2 min** (`D-26` (3), y `Esclavo/include/reloj.h` lo dice: *«sin radio, al reves»*). Esa frase tenia trabada a `A-16`.
| ~~**A-16**~~ | ✅ **RESUELTA el 14/09: `D-22` va la ULTIMA, va SOLA y no se carga sin una tarjeta delante.** Se acepta a cambio que el tope de 48 h sin sincronizar **pueda cumplirse hasta ~1 h 10 min antes o ~1 h despues** —de **ficha del fabricante, NO medido en tarjeta**, y el desvio va en las dos direcciones—. La contradiccion de su fila se deshizo sola: **la siembra existe desde `68dd2c5` y va cada 2 min**, no cada hora, asi que `Y1` **ya no decide los 29 s del cruce**. El comportamiento, en `SPEC_7` §5.1 | — | — | — |
| ~~**A-1.bis**~~ | ✅ **RESUELTA el 14/09 por `D-33`: SÍ se deroga**, y sólo en su mitad de *«nunca al revés»* — la pluma **sigue a la luz para SUBIR** y la cámara sólo puede **retener la bajada**. Construida y en `main` (`363e375`). Lo que queda de esta pregunta no es una decisión: son los **modos de fallo que el veto CREA**, y viven en `roadmap` **1.42** | — | — | — |
...
| ~~**A-8**~~ | ✅ **CERRADA el 12/09 por el responsable —*«ciérrala»*— y ya estaba resuelta en el papel: los DOS `Arming Schedule` a 24×7.** Medido ese día en la guía que va al poste: lo dice en **tres sitios**, «24 horas los 7 días, **en la regla y en la salida**», con su motivo escrito —*«si no, de noche o en fin de semana la cámara no dispara y nadie se entera»*—. Y es la única configuración que **no depende del reloj de la cámara**, que era justo lo que abría esta fila: si el armado no cubre las 24 h, un reloj desincronizado corre la ventana | — | ya no bloquea nada |
...
| **D-23** | 🔴 **LA APP NECESITA UNA PANTALLA PROPIA DEL POSTE 2, distinta de la que tiene hoy.** Hoy la app enseña el estado del cruce **leyendo las tramas que el Esclavo reporta POR RADIO**; lo que falta es lo que se ve **cuando el teléfono se conecta por Bluetooth DIRECTAMENTE a ese poste**: el diagnóstico de esa punta por sí misma | 07/09 · decidida por el responsable | *«la app, cuando se conecta a ese lado del Esclavo vía Bluetooth, podrá diagnosticarlo; y ojo, casi que esto es una pantalla para Esclavo, diferente de lo que hoy hace la app»*. **Y resuelve `D-21` sin tocar el firmware:** el escenario de la hora mentirosa dentro del Degradado **tardaría MESES** en darse —el Maestro reenvía la hora periódicamente—, así que **no hace falta que el equipo decida solo: hace falta poder DIAGNOSTICARLO cuando alguien vaya**. ⚠️ **Es consecuencia directa de `D-16`** —sin teléfono no hay forma de operar el equipo—: si la app es la única superficie, **el poste 2 no puede ser sólo un renglón dentro de la pantalla del poste 1**. 🔴 **07/09 noche — DECIDIDA Y SIN CONSTRUIR, y con un ancla decorativa retirada:** `b36ffb7` puso dos comentarios —uno en `despachador.cpp` y otro en el `bluetooth.cpp` del Esclavo— y con eso `decisiones_01_anclas` dejaba de contarla como ausente; **revertido en `5d0a0b9`**. **Es una decisión de la APP y la app no tiene ni una línea:** medido sobre **toda** la rama, `app.js` e `index.html` no aparecen en el diff, y **las CUATRO copias de `app.js` del árbol** —`App_Semaforo/`, `www/`, `android/app/src/main/assets/public/` y `android/app/build/intermediates/assets/debug/public/`— son **idénticas por hash** (`md5 b09dcc85…`, 317.993 B). *(El parte hablaba de tres: son cuatro.)* ~~🔴 **Y ANTES DE CONSTRUIRLA HAY QUE ELEGIR LA VÍA, que NO está elegida → `A-14`**~~ — 🟢 **CADUCADO: la vía se eligió el 07/09 por la noche y se confirmó el 08/09 — `$EVENT` nuevo, emitido también al conectar** (`A-14`, tachada en el índice y resuelta en su cuerpo). ~~Lo que falta es construirla~~ 🔴 **12/09 NOCHE — SE INTENTÓ CONSTRUIR Y EL AGENTE PARÓ CON RAZÓN: FALTA UNA COSA MÁS QUE NO ES TECLADO.** La **vía** sigue elegida (`$EVENT`, no campo en el `$STATUS` — reconfirmado hoy: un campo nuevo **no entra por 3 bytes**, el peor `$STATUS` del Maestro son 151 caracteres contra los 154 que guarda `payload[155]`, y `documentos_03` obliga a que esté en las **dos** puntas). **Lo que NO está elegido es el MECANISMO de «al conectar», y es que el STM32 NO PUEDE SABERLO:** el único que ve el teléfono es el ESP32 (`spp.hasClient()`). Dos formas medidas, y la elección es del responsable porque una de ellas toca una barrera de seguridad: **(i)** un aviso nuevo **ESP32 → STM32**. Sería la **TERCERA línea que el accesorio origina hacia el micro**, y `esp32_05_no_origina` lo condiciona **por escrito** a *«una decisión escrita en `DECISIONES.md`, no un comentario»*, porque es *«una orden que el accesorio manda por su cuenta a un micro que gobierna un cruce y que no valida quién habla»*. Con la fila escrita —y su gemela anti‑suplantación, como la de `HORA_ESP32`— el trabajo es pequeño. **(ii)** que el STM32 **no se entere de nada** y el `$EVENT` salga **periódico a cadencia baja**: no toca el ESP32, no toca `esp32_05`, no toca ninguna barrera, y el caudal lo aguanta (**51,8 %**, 497 B de 960 B/s) — a cambio, el técnico lo ve **al cabo de la cadencia**, no al instante, y contradice el *«no gasta periódico»* con el que se eligió la vía. ⚠️ **Mientras no se elija, `D-23` no se construye, y por eso `decisiones_01_anclas` la sigue acusando con razón: está decidida y sin construir** (`CLAUDE.md` §1 — la cuenta de ese rojo dice que se está decidiendo más rápido de lo que se construye, y **no se apaga poniendo el ancla**) | «la app enseña el cruce y con eso basta para las dos puntas» |
...
| **D-28** | 🔴 **EL PLAZO DE CADUCIDAD DE LA HORA AGUANTA DOS SIEMBRAS PERDIDAS, NO UNA — y la cadencia son 120 s EXACTOS.** Dos cosas, y las dos cierran un «~» que estaba abierto. **(1) `D-26` (2) queda fijada al segundo: la cadencia es `120000 ms`**, no «~2 min». El número no estaba escrito en la decisión —salía de la medida de la fila 2.10 del roadmap— y por eso el agente que la construyó el 12/09 lo devolvió en vez de elegirlo. **(2) El plazo deja de derivarse del RELEVO y pasa a derivarse de DOS SIEMBRAS PERDIDAS**, que es el caso peor de los dos: `3 × cadencia` inflado por el HSI = **369 s**, cuantizado a **10 s** de deriva concedida → **`HORA_CADUCA_MS` = 400 s** (hoy 280). Con esto una punta con el `J17` mudo aguanta dos siembras seguidas perdidas sin irse a ámbar; hasta hoy la segunda la tumbaba y **no volvía sola**. ⚠️ **LO QUE ESTO CUESTA, MEDIDO y aceptado por el responsable:** el desfase relativo que el cruce puede acumular sube de 16 s a **22 s**, contra un aguante de **29 s**, así que **el margen que queda para lo que difieran los dos `DS3231` baja de 13 s a 7 s**. Sigue siendo positivo y `reloj_04` lo recalcula en cada corrida; el día que deje de serlo, el pack lo dice. **La regla de minimalidad cambia de sujeto: el plazo sigue siendo el MENOR que cubre su caso peor, pero el caso peor ya no es el relevo (271 s) sino las dos siembras (369 s)**, y el `static_assert` del techo se rehace contra el término nuevo — el del relevo se conserva porque vigila otro término | 12/09 · decidida por el responsable | La opción contraria era el plazo mínimo (280 s, margen 13 s), que era la recomendación de esta sesión: entre «el cruce se para y hay que ir al poste» y «se estrecha el margen contra dos verdes a la vez», lo segundo es lo que mata. **El responsable eligió lo primero con la medida delante**, que es suyo decidir: un poste que se va a ámbar y no vuelve solo es una avería que se paga con un viaje, y a esta cadencia dos siembras perdidas seguidas dejaron de ser improbables. Las cifras salen de los ayudantes ya probados del banco -`_aguante()` de `esp32_13` y `_relativa_s()` de `reloj_04`-, no de una cuenta a mano | de `D-26` (2), el «~» de «~2 min»; de `D-21` (1), que el plazo se derive del relevo |
| **D-29** | 🔴 **N-20 SE RECONSTRUYE: UN CORTE DE LUZ NO PUEDE MATAR LA REANUDACIÓN DEL DEGRADADO.** Medido el 12/09: desde `N-162` (11/09) la siembra ya no escribe el RTC hardware del STM32, así que `reloj_setup()` deja `horaValida` en `false` **tras cada corte** y `degradado_reanudarTrasCorte()` cierra por su primera puerta — **y en ese mismo arranque borra el indicador de la pila** (`respaldo_guardarDegradado(false)` de la rama `!sigueVigente`), de modo que la hora del ESP32, que llega en el `loop()` un segundo después, ya no tiene nada que reanudar. **Esto NO se decidió: es un efecto colateral de `D-20`/`D-26` que nadie vio**, y el comentario del firmware que lo atribuye a «sin cristal» se queda corto — pasa **también con el cristal vivo**. Se reconstruye: **el indicador de la pila no se borra hasta después de la primera siembra del arranque**, para que la reanudación pueda decidirse con la hora que el ESP32 acaba de dar. ⚠️ **La barrera que NO se toca al hacerlo:** la segunda puerta —el límite duro de 48 h desde la última sincronización— sigue mandando, porque es la que impide reanudar sobre una marca que ya no significa nada; y sigue sin haber entrada automática al Degradado (`SFTY-21`: activación MANUAL). **Estrecha, PERO NO CIERRA DEL TODO, la divergencia de las DOS FLOTAS**: hoy una tarjeta cuyo RTC escribió un firmware anterior al 11/09 sí reanuda y una recién grabada no, con el mismo binario y sin que nada en el `$STATUS` lo distinga. 🔴 **AQUÍ DECÍA «cierra la divergencia» Y ERA FALSO — lo midió el agente que la construyó, el 12/09, y se corrige antes de que nadie lo lea como cubierto:** con el cristal `Y2` **muerto** (`N-17` lo midió en una tarjeta), `reloj_setup()` deja `rtcOperativo` en `false`, `reloj_contadorSegundos()` devuelve **0** a propósito (`N-160`) y `respaldo_horasDesdeSync()` responde `CADUCADA`, así que **cierra la SEGUNDA puerta y el diferimiento ni llega a activarse**. `D-29` alcanza sólo a las tarjetas cuyo `Y2` oscila. Es correcto que sea así —extenderlo sería reanudar sobre una marca caducada, justo lo que la segunda puerta impide—, pero **no se puede publicar como cerrado**. ⚠️ **Y una condición añadida por el responsable el 12/09, al ver el caso que trajo el agente:** la reanudación diferida **consulta `mando_ambarLocal()` y tira el permiso si está puesto**. Con el mando desmontado (`D-1`) esa bandera no se arma nunca y la guarda no dispara; existe porque `J16` p5/p8 siguen vacíos y pelados, y su modo de fallo —no reanudar— es el de hoy | 12/09 · decidida por el responsable | Se descubrió al arreglar el bloque D del arnés de dos puntas, que medía la reanudación **contra un modelo de RTC que el firmware ya no escribe** y por eso la daba por viva (`N-162` `H8`, `roadmap.md` 1.16(c)). El arnés ya trae el control que lo demuestra: con el marcador del RTC puesto —un equipo anterior al 11/09— el mismo escenario SÍ reanuda, o sea que la reanudación de `N-20` está viva y lo único que la apaga es la escritura retirada | que `N-20` haya muerto en el Esclavo, que era el estado de hecho desde el 11/09 sin que nadie lo decidiera |
| **D-30** | 🔴 **EL LCD Y EL MANDO A/B/C/D SALEN DEL FIRMWARE. Se deroga la segunda mitad de `D-1`.** El hardware de los dos se retiró —la pantalla el 28/08 (`D-17.bis`), la botonera y el mando de relés el 05/09 (`D-1`)— y **el software se quedó a propósito**. Hoy el responsable lo revierte: *«ya ni los capturamos… arrastrar esas funcionalidades hoy pesa»*, *«hasta el sw lo quitamos»*. **Se retiran del firmware `lcd.cpp`, `menu.cpp` y `mando.cpp` de las dos puntas, y la lectura de flancos de `BOTON1`/`BOTON2` que los alimenta.** ⚠️ **NO ES UN BORRADO MECÁNICO, y `D-1` dejó escrito por qué —esos motivos siguen siendo ciertos y son ahora el ALCANCE del trabajo, no una objeción—:** **(1)** `mando_ambarLocal()` tiene **cinco llamadas vivas** —tres vetos en `Esclavo/src/main.cpp`, dos decisiones de `CANCELAR_AMBAR` en su `bluetooth.cpp`— y su veto **es `SFTY-21`**: retirar sólo el armador **no deja los `if` inertes, los deja ABIERTOS**. Cada uno de los cinco se resuelve **explícitamente**, no se deja caer. **(2)** Desde el 12/09 hay una **sexta**: la guarda de `D-29` en la reanudación diferida. **(3)** **Trece packs** leen constantes de `mando.cpp` **en el import**, así que se caerían en **`ABORTADO`, no en rojo** — y un `ABORTADO` no dice nada del firmware. **(4)** El **arnés de pantalla** mide hoy `287/287` sobre `lcd.cpp` y `menu.cpp`: se retira con ellos, y eso **baja la cuenta de la compuerta a propósito** — se dice en el acta, no se disimula. ⚠️ **Lo que NO sale:** `botones.cpp` sigue leyendo las **cámaras** por los mismos caminos, y `J16` p5/p8 seguirán **físicamente vacíos y pelados**: retirar el reconocedor de secuencias hace que un puente ahí deje de mover el cruce, que es una mejora de seguridad, **pero no sustituye a la instrucción de no cablearlos**. 🔴 **12/09, MEDIDO ANTES DE TOCAR NADA — Y ES MUCHO MÁS GRANDE Y MÁS CARO DE LO QUE ESTA FILA DECÍA. NO SE HA CONSTRUIDO: se paró aquí a propósito.** **(a)** De las seis llamadas, **la que ABRE PASO es la del `CMD_GO_GREEN`** (`Esclavo/src/main.cpp`): hoy veta el verde con el ámbar del mando puesto; sin la bandera, **el `GO_GREEN` se obedece**. Y la de `D-29` abre un nivel por encima —deja reanudar el Degradado, el único modo que da verde sin confirmar la otra punta—. Las otras cuatro abren hacia rojo o no tocan luz. ⚠️ **Por eso el armador y los seis lectores van en EL MISMO COMMIT: quitar sólo `mando.cpp` y dejar los `if` deja esos dos ABIERTOS de verdad**, y hoy la bandera **sí puede armarse** porque `J16` p5/p8 están vacíos, pelados y leídos en `INPUT` crudo. **(b)** 🔴 **`Maestro/src/menu.cpp` NO es sólo pantalla: `menu_setup()` llama a `coordinador_forzarMenu()`**, el todo‑rojo de las dos puntas, **alcanzable HOY desde la app** (`SET_MODO:MENU`). Borrarlo **borra ese todo‑rojo**: hay que mudarlo, no dejarlo caer. **(c)** 🔴 **Se lleva por delante parte de `semaforo.cpp`, que es `SFTY-2`:** `mando.cpp` es **el único escritor** de `senalActiva`, así que tres funciones del Maestro se quedan sin llamador y en el Esclavo `!semaforo_senalEnCurso()` pasa a ser **una guarda que sólo sabe dar una respuesta** (§6.2). **(d)** Y **el precio que hay que decir en voz alta: después de esto NINGÚN instrumento del proyecto puede cazar un defecto en la interceptación de `escribirPines()` por señal** —hoy lo único que la ejercita es `mando.cpp`—, mientras que **el código interceptado se QUEDA** en `semaforo.cpp` como un camino sin ejercicio ninguno. **(e)** Alcance real: **no 13 packs sino 10 que abortan en el import y ~22 en total**, cuatro de ellos a **borrar** porque sólo miden lo retirado; borrar `lcd.cpp` **aborta la guarda de rutas de la compuerta entera** (un pack la declara a propósito para que la censen); y el lote **tiene que incluir `Validacion_Automatico/`** —sus tres arneses compilan `mando.cpp` real y uno incluye los `lcd.h`/`menu.h` reales— **y `semaforo.cpp`**. ✅ Lo que **sí** sale limpio: `botones.cpp` **no está trenzado** —mando y cámaras no comparten ni una variable—, y **`MODO_HORA` ya está muerto hoy** (su único armador vive tras un `botonAceptar()` que es `return false` desde el 31/08), así que se va con el menú sin cambiar conducta. ⚠️ **`MODO_ALCANCE` NO**: la app lo arma. ⬇️ 🟢 **REAFIRMADA POR EL RESPONSABLE EL 14/09** *(«eliminamos las botoneras A, B, C y D; ahora es por app»)*, **y su precio se midio ese mismo dia y es MENOR del que esta fila supone**: retirar el mando **no** toca la barrera de salidas —la bandera que intercepta las luces se queda en falso y su guarda deja de disparar, o sea camino normal— y el todo-rojo de las dos puntas **no** depende del menu: tiene **tres** llamadores y dos se alcanzan desde la app. Lo que si queda es codigo de interceptacion **sin nadie que lo ejerza**: sale entero o se queda vigilado (`SPEC_4` §3.bis).

🔴 **El ahorro de flash NO se escribe aquí hasta medirlo**: `CLAUDE.md` §10 exige medir **los DOS extremos** por fichero objeto sobre `firmware.map`, y el Maestro está al 89,5 % con 5.900 B libres — la cifra sale del trabajo, no de esta fila | 12/09 · decidida por el responsable | Motivo suyo, literal: *«arrastrar esas funcionalidades hoy pesa»*. Y el motivo técnico que lo hace algo más que limpieza: hoy el firmware carga **dos subsistemas cuyo hardware no existe** en el micro con menos margen del proyecto. ⚠️ **`D-1` no se retira entera: su primera mitad —«el equipo se opera SÓLO POR APP»— sigue vigente y es la que manda**; lo que se deroga es «y su CÓDIGO no se toca» | de `D-1`, su segunda mitad («el código se queda»); de `D-17.bis`, el residuo de software de la pantalla |
| **D-33** | 🔴 **LA CÁMARA VETA LA BAJADA DE LA PLUMA, Y LA PLUMA BAJA 3 SEGUNDOS DESPUÉS DEL ROJO** *(los 3 s los FIJA el responsable; no salen de una cuenta)*. Veta **cualquiera** de las dos cámaras, el veto es **local** —no para el ciclo— y **ante error o falsa alarma la barrera NO baja**: se avisa, diciendo cuántos segundos lleva retenida. **Traducirlo a «revise el ajuste de la cámara» es trabajo de la app y está PENDIENTE.** **El umbral de «muchas falsas alarmas» se cerró el 14/09: NO HAY UMBRAL — basta UN veto que sobreviva al todo-rojo más largo, porque un vehículo despeja en segundos.** **Pendientes de construir: que la app traduzca ese aviso a «revise el ajuste de la cámara», y DOS interruptores de administrador por poste** —*sacar la cámara del veto* y *sacar la barrera de servicio*—, que sobreviven a un corte y se publican. El comportamiento entero está en **`SPEC_5` pág. 1 §4**; el porqué, en `roadmap` 1.41 | 14/09 · decidida por el responsable | **Cierra `A-1.bis`** y `SPEC_1` §12.1 · desbloquea la fase 2 de `D-13` | **`SFTY-28`** en su *«nunca al revés»*, y la premisa de `semaforo.cpp` que decía lo contrario *(«una pluma arriba con la luz en rojo es PEOR que no tener barrera»)*, derogada por el responsable con su nombre: *«esas barreras son casi de adorno; el que manda es el semáforo y su estado»* |
| **D-34** | 🔴 **LA VENTANA AMBAR CONTRA VERDE SE CIERRA EN LAS DOS PUNTAS.** El Esclavo suelta su verde `AVISO_AMBAR_TIMEOUT_MS` antes de su silencio, la repeticion del `GO_GREEN` ya no lo refresca, y lo dice en el param del `PONG`; el Maestro no entrega `GO_GREEN` sin un `PONG` mas joven que `LATIDO_MS` y, al oir ese aviso, reanuda por la puerta de `N-163`. La alarma de reintentos agotados dice `CAMBIO_A_ROJO`. Especificacion: `roadmap.md` 1.39 | 15/09 · **delegada** por el responsable en un revisor independiente (*«si necesitas definir algo preguntale a fable»*) | medido: hasta 23 s de ambar contra verde en carril unico (1.39) | en `protocolo.h`, «lo que se alarga es una espera, no un riesgo»; el texto `CAMBIO_A_AMBAR`. No deroga `N-163` ni los 25 s |
| **D-32** | 🔴 **LAS CUATRO RESPUESTAS DEL 13/09, QUE DESTRABAN TRES ROJOS Y UN MODO.** El responsable contesta las cuatro preguntas que quedaron abiertas la noche del 12/09, y cada una **deroga un trozo de la fila que la planteaba**. **(1) `D-30` SE RECORTA: SALE SÓLO EL LCD. El mando A/B/C/D SE QUEDA en el firmware** —*«solo retirar el lcd»*—. Es la alternativa que la medida recomendaba: el mando es la mitad **trenzada con la seguridad** (`mando.cpp` es el único escritor de `senalActiva`, o sea `SFTY-2`), y retirarlo dejaría `escribirPines()` **sin ningún instrumento que ejercite su interceptación**. ⚠️ **Y esto NO cierra `J16` p5/p8**: siguen vacíos, pelados y leídos por **dos** caminos. **(2) `D-23` se construye por la vía del `$EVENT` PERIÓDICO** —*«creo que event, revísalo y elige uno»*, elección delegada y medida abajo—. **(3) `D-31`: MÁS REINTENTOS** —*«14 o más»*—, o sea que el responsable **acepta que la alarma verdadera tarde 14 s** a cambio de menos falsas alarmas a 8 km. **(4) `D-14` NO SE INSTALA EN ESTE DESPLIEGUE** —*«revisa el .html, eso es lo que usamos»*—. | 13/09 · decidida por el responsable | **(1)** La medida que la sostiene está en `D-30` y en `roadmap.md` 1.30: de las seis llamadas a `mando_ambarLocal()`, la del `CMD_GO_GREEN` del Esclavo **ABRE PASO**, y retirar sólo el armador la deja **ABIERTA, no inerte**. Sacando sólo el LCD ese riesgo no se toca. 🔴 **Pero `menu.cpp` NO es sólo pantalla y por eso hay que medirlo antes de tocarlo: `menu_setup()` llama a `coordinador_forzarMenu()`, el todo‑rojo de las dos puntas, alcanzable HOY desde la app.** Si `menu.cpp` cae con el LCD, ese todo‑rojo necesita otra puerta **en el mismo commit**. · **(2) Por qué el `$EVENT` periódico y no el aviso ESP32→STM32, medido:** la otra vía sería **la tercera orden que el accesorio origina hacia el micro**, y `esp32_05_no_origina` la condiciona por escrito a una fila de esta tabla, *«porque es una orden que el accesorio manda por su cuenta a un micro que gobierna un cruce y que no valida quién habla»*. **La periódica no toca ninguna barrera**, no toca el ESP32, y el caudal lo aguanta (**51,8 %**, 497 B de 960 B/s). Coste aceptado: el técnico lo ve **al cabo de la cadencia**, no al instante. · **(3)** El número **ya no se deriva del presupuesto de radio** —eso era falso y se corrigió el 12/09—: sale de este reloj de pared. `(1 + N) × AVISO_AMBAR_TIMEOUT_MS`; con **`AVISO_AMBAR_REINTENTOS = 3`** son **14 s**. ⚠️ **Los dos `static_assert` de `coordinador.cpp` NO dejan pasar hoy más de 1**, así que hay que **rehacerlos contra el modelo coherente** —el que el propio firmware implementa: las esperas corren en paralelo y sólo se serializa el cable— **en el mismo commit que el número**. · **(4)** Medido sobre `05_Funcional/Camaras_Sisga_4x.html`, que es la guía que sigue el instalador: cablea **`J16`** (48 menciones), **`J15`** (15) y nombra `J14` (9); de **`J9`, `J11` y `J13`: CERO**. Y sólo cablea la **salida** de alarma de la cámara (`1A`/`1B`), nunca su **entrada**, que es lo que `D-14` pedía. **Misma forma que `D-27` (2) con `J14`: la decisión no borra la capacidad, dice que en este despliegue no se instala.** | de `D-30`, la retirada de `mando.cpp` (el LCD sí sale) · de `D-23`, que el mecanismo estuviera sin elegir · de `D-31`, que `AVISO_AMBAR_REINTENTOS` valga 1 · de `A-16`/`§2.4`, la urgencia de elegir entre `J9`/`J11`/`J13` para `D-14` |
| **D-31** | 🔴 **EL AVISO DE ÁMBAR ENTRE POSTES SE ACUSA.** Hoy, cuando el Poste 2 entra en ámbar de emergencia, manda `CMD_AMBAR_ESCLAVO` al Poste 1 **sin esperar acuse y sin reintento**, y eso deja una avería que **nadie puede ver**: si se rompe **sólo el transmisor** de esa punta —oye pero no habla—, la punta **no tiene forma de saberlo** (su único dato de radio es el silencio de lo que RECIBE, y el Maestro le sigue hablando), el técnico ve un `$ACK` idéntico al de la radio sana, se va, y **el Poste 1 sigue dando verde hacia un carril cuyo otro extremo está en ámbar**. Lo mide el bloque `H4` del arnés de dos puntas, que lo publica como **nota que no cuenta** porque **ningún firmware de esa punta puede aprobarlo solo**. Se construye el acuse: el Poste 1 contesta y el Poste 2 lo espera, del mismo tipo que el `ACK_RED` sin identificador de `G9`. ⚠️ **Toca el CONTRATO DE LA RADIO en las DOS puntas** —`{Maestro,Esclavo}/include/protocolo.h`, los dos `bluetooth.cpp` y `coordinador.cpp`—, que es lo caro, y por eso estuvo esperando decisión. ⚠️ **Y hay que medir dónde muerde de verdad, que es al revés de como se contó:** la rama principal del ámbar de emergencia es `if (!degradado_gobiernaLuz())`, o sea que **actúa en OPERACIÓN NORMAL, obedeciendo al Maestro**; sólo cuando el Degradado gobierna la luz el ámbar **se encola** y espera a la salida por todo‑rojo. El hueco muerde en el caso **común**, no en el raro.

🔴 **CÓMO SE CONSTRUYE — ESPECIFICADO EL 12/09, Y ESTO NO SE VUELVE A PREGUNTAR.** El primer diseño («que el Poste 1 acuse siempre») **se descartó porque construía algo PEOR que el defecto**, y las tres correcciones salieron de preguntas del responsable. Queda cerrado así:

**(1) El acuse se espera SÓLO DEL PRIMER AVISO.** Al llegar el acuse, el Esclavo **guarda** que el Poste 1 ya lo sabe; las pulsaciones siguientes contestan *«ya estaba avisado»* y **no esperan nada**. **Por qué:** el aviso es justamente lo que manda al Maestro a `MODO_AMBAR`, y allí **tiene prohibido transmitir** (`SFTY-21`, `coordinador_escucharEnAmbar()`: *«callar no es no oír»*). Un diseño que esperase acuse en cada pulsación diría *«nadie me oyó»* **con la radio sana** cada vez que alguien pulsa dos veces — que es lo normal en un poste—, y **eso enseña al técnico a ignorar la única señal que esta decisión existe para darle**. El caso ya está montado en el arnés: es `H2`. ⚠️ **Así `SFTY-21` NO se toca**: el Maestro sólo contesta mientras cicla, que es cuando puede.

**(2) EL CANCELAR ENTRA EN EL MISMO LOTE, y no sólo porque su ventana sea peor** —al cancelar el Maestro está callado en ámbar, así que la red de `N-142` (agotar reintentos → `C_FALLO`) **no aplica**: *«un Maestro que no pregunta nada no puede agotar nada»*, y con el transmisor muerto el cruce se queda en ámbar hasta que alguien camine al Poste 1—. **Es que el cancelar es lo que BORRA esa memoria**: sin él, la próxima vez el Esclavo no esperaría acuse cuando sí debe. Armado y cancelado son **la misma máquina**; partirlos deja un estado que miente.

**(3) TIEMPOS Y REINTENTOS, con lo que la distancia sí cambia y lo que no.** El plazo de espera es **`TIMEOUT_ACK_MS` (3,5 s)** —no un número nuevo: es el que el firmware ya usa como «un viaje de radio con margen»—. **Y el ámbar NO espera**: se enciende al instante y el teléfono recibe su `$ACK` inmediato, con el desmentido posterior como evento (el patrón de `CMD_ACK_DEMANDA`, `N-130`: *«no puede esperar: bloquear el bucle por una radio de 2,4 kbps es peor»*). ⚠️ **El responsable pregunta si eso vale a 8 km, y el cruce es MÓVIL. Medido, y ya estaba escrito: la distancia aumenta la probabilidad de PÉRDIDA, no la latencia** —a 8 km la señal tarda 27 µs más—, **y contra una trama perdida sirve REPETIR, no esperar**. Pero eso trae el límite duro: **una trama perdida se ve igual que un transmisor roto**, o sea la misma falsa alarma con otra causa, y **el presupuesto de radio ya va por 20,8 s de un techo de 25** (`SFTY-7`). **Quedan ~4 s: cabe UN reintento, no cinco** — y **el número se DERIVA de lo que quede libre, no se elige**. ~~lo recalcula `costura_09_presupuesto_radio` en cada corrida~~ 🔴 **ESA FRASE ERA FALSA y se corrige el mismo día, al construirlo: `costura_09` NO recalcula el reintento del aviso y NO PUEDE** —lee sólo `Maestro/src/coordinador.cpp` y `protocolo.h`, y modela **el ciclo**, no el aviso—. La derivación vive donde sí manda: **dos `static_assert` en `coordinador.cpp`** con la fórmula exacta de `costura_09` (25,0 − (3,0 + 5×3,56) = **4,2 s libres**; una espera = 3,56 s ⇒ cabe 1, no 2). **Y se probó por los dos lados, que es lo que demuestra que está derivado y no elegido: con `2` NO COMPILA** («no caben en lo que queda del presupuesto») **y con `0` tampoco** («cabe UN reintento más»). ⚠️ **Y queda una tensión abierta que hay que resolver, no tapar: los DOS presupuestos del repo no coinciden.** El `static_assert` de `N-163` compromete **24.000 de 25.000 ms** —sólo 1.000 libres— y con **esa** contabilidad el reintento **no cabría**. Miden cosas distintas —aquél suma un `TIMEOUT_ACK_MS` que no es tiempo de aire, y suma en serie porque lo hace el mismo extremo; el aviso lo manda el otro y sólo serializa 2×60 ms de cable—, y se usó el borde de `costura_09`, que es **el que el responsable tuvo delante al decidir**. **Dos presupuestos que no casan sobre el mismo techo de 25 s es una fila pendiente, no un detalle.**

> 🔴 **CORRECCIÓN DEL 12/09 POR LA NOCHE, MEDIDA, Y ES CONTRA MÍ: EL `1` NO ESTÁ DERIVADO. ESTÁ ELEGIDO.**
> Lo primero, porque es lo que decide si alguien tiene que salir corriendo: **el techo de 25 s AGUANTA y
> `SFTY-6` no se rompe.** Esto no cambia ni una línea del firmware que está en `main`, y **`D-31` sigue
> siendo la decisión correcta**. Lo que se cae es *mi justificación del número*, que es peor que un
> número malo: un número elegido y llamado derivado **bloquea la pregunta**, porque desde ahí ya nadie
> mira la causa (`CLAUDE.md` §8.2).
>
> **La incoherencia, y está DENTRO DEL MISMO FICHERO, a 25 líneas de distancia.** El párrafo de
> `coordinador.cpp` que elige el borde razona que las esperas del aviso **corren EN PARALELO** a las del
> ciclo y que *«lo único que se serializa en el aire medio dúplex es el tiempo de cable, 2 ×
> `ENVIO_TRAMA_MS`»* —**120 ms**—. Y el `static_assert` de más abajo **cobra 3.560 ms por reintento**.
> No son dos lecturas de lo mismo: son **dos modelos incompatibles**, y la cuenta que produjo el `1` toma
> el **coste unitario** de uno con el **recuento** del otro —cobra los reintentos y **no cobra el intento
> inicial**—. Bajo el modelo del propio párrafo caben **34**; bajo el modelo serie completo **no cabe ni
> uno** y el número tendría que ser **`0`**, que anula `D-31` entera.
>
> ⚠️ **Y por eso «se probó por los dos lados: con 2 no compila y con 0 tampoco» NO demuestra lo que yo
> dije que demostraba.** Las dos mitades salen del mismo híbrido, así que el `1` es el único valor que
> compila **contra un modelo que no describe nada**. Es exactamente §6: la excepción es el instrumento de
> verdad, y aquí la excepción era la frase «los reintentos del aviso gastan del presupuesto de radio»,
> que nadie había medido — la escribí yo.
>
> 🔴 **LO QUE ESTO DEVUELVE AL RESPONSABLE, y es una pregunta de verdad, no una que ya esté contestada
> arriba.** Le dije que el reintento era **uno porque el presupuesto de radio sólo dejaba ~4 s**. Eso es
> falso: **el presupuesto no es la restricción.** La restricción real es **el reloj de pared antes de que
> el Poste 2 declare que el otro no contesta**, que hoy son `(1 + 1) × 3,5 s` = **7 s**. Con **2**
...
para Intrusión)~~ 🔴 **DEROGADO POR `D-27` (11/09): el objetivo es ☑ Vehículo · ☐ Humano.**
*(Este párrafo siguió prescribiendo lo contrario que la fila `D-27` durante un día, 437 líneas
por debajo de ella y sin una sola marca. Lo cazó la auditoría de cámaras del 12/09, y **no lo
podía cazar ningún instrumento**: `documentos_06` excluye `DECISIONES.md` a propósito. Es el
bucle de `CLAUDE.md` §11.1 dentro del documento que gana a todos los demás — un agente que
hiciera `grep D-13` para configurar una cámara leía «sin filtro de objetivo».)* ⚠️ **Lo que de
aquel paréntesis SIGUE SIENDO CIERTO y hay que conservar: `Detection Target` NO está
documentado para Intrusión en el manual oficial** —su «5. Set rules» lista sólo `Sensitivity`
y `Threshold`—, aunque la ficha sí anuncia clasificación humano/vehículo. Por eso la guía de
campo lleva la salida escrita: si la casilla no aparece, se anota, se avisa y se sube el mínimo
del `Size Filter`. · `Trigger Alarm Output` **sólo en esta regla** (el enlace es común a todas
