...
| **A-14** | 🔴 **Por qué VÍA viaja el diagnóstico del poste 2** que pide `D-23`: **campo nuevo en el `$STATUS`**, **`$EVENT` nuevo**, o **comando nuevo con su `$ACK`** | alta — **es lo PRIMERO de `D-23`**, y sin ella no se escribe una línea | **`D-23` entera**: la pantalla propia del poste 2. Las tres vías están **medidas por buffer** en `05_Funcional/14_Manual_App_Movil_IOT_VIAL.md` §5.8.6 — aquí no se copia la cuenta, se apunta. **El presupuesto que decide no es el del Esclavo: es el del MAESTRO** |
