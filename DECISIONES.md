...
|---|---|---|---|
| ~~**A-12**~~ | ✅ **RESUELTA el 05/09 y ARREGLADA en el firmware.** Pasa a **`D-19`**. El corte de 15 s ya no existe: `grep 15000 modo_inteligente.cpp` sólo lo encuentra **dentro de dos comentarios que cuentan que estuvo ahí** | ~~la compuerta está en ROJO por esto~~ — **la compuerta está en `20 PASS · 0 FALLA`** (acta del 07/09) y el banco en `1180/1180` | nada. ⚠️ **Lo que SÍ sigue parado es la condición de `D-19`**, que no la cierra un commit |
