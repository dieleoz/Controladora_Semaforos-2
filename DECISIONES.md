...
| **A-9** | 🔴 **HAY DOS RELOJES POR CRUCE Y NADA LOS SINCRONIZA NI LOS COMPARA** | Medido por dos vías: los dos ESP32 **no se hablan** —sin WiFi, sin ESP-NOW, sin radio: sus únicos dos objetos de E/S son su propio STM32 y el teléfono—, y el único canal horario del equipo va por LoRa **entre los dos STM32**, cuyos relojes están muertos (`Y2`, N-17). **Cada DS3231 corre libre desde que un técnico le tecleó la hora, en ese poste, en esa visita.** ⚠️ **Hoy nos salva un cristal roto, no un diseño**: el Degradado calcula con el reloj del STM32, que está bloqueado, así que el margen medido (29 s de aguante, factor 1,44) **no lo gasta nadie**. **El día que AB-4 cuelgue el Degradado del DS3231 eso cambia de dueño:** la deriva mejora ~50× (TCXO ±2 ppm frente a los ~100 ppm de `Y2`), **pero el desfase INICIAL no tiene cota ninguna** — es «cuánto distaban las dos visitas», medido por el reloj de dos teléfonos. **Decisión pendiente: si ese desfase necesita cota y quién la mide** |
